...
class EngineAPI(object):
    ...
    def get_celery_task_status(self, task_id):
        ...
        try:
            response = self.api_client._request_with_params(GET,CELERY,self.api_client.AUCTION_API, params, self.token, None)
            return response
        except:
    ...
    def get_processing_tasks(self):
        try:
            result = self.api_client._request_without_params(GET,PROCESSING_TASKS,self.api_client.AUCTION_API, self.token)
            return result
        except:
    ...
    def send_jobs(self, job_data):
        try:
            response = self.api_client._request_with_params(POST,JOBS,self.api_client.AUCTION_API,job_data,self.token, None)
            return response
        except:
